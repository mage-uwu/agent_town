...
function drawFrame() {
  if(action!=='idle')frames=animationCache[frameIndex];
  for(const dir of DIRECTIONS)canvas(frames[dir],$(`view-${dir}`));
  selectDirection(direction);$('frame').value=String(frameIndex);
  $('frame-label').textContent=action==='idle'?'STANDING':`${frameIndex+1}/8 · ${PHASES[frameIndex].toUpperCase()}`;
}
function pauseAnimation() {clearInterval(playback);playback=null;$('play-pause').textContent='▶';$('play-pause').setAttribute('aria-label','Play animation');}
function playAnimation() {
  if(action==='idle')return;pauseAnimation();$('play-pause').textContent='Ⅱ';$('play-pause').setAttribute('aria-label','Pause animation');
  playback=setInterval(()=>{frameIndex=(frameIndex+1)%ACTION_FRAMES;drawFrame();},1000/Number($('speed').value));
}
function refresh() {
  model=buildModel(player);animationCache=action==='idle'?null:animationFrames(player,action);
  if(action==='idle')frames=Object.fromEntries(DIRECTIONS.map(d=>[d,renderModel(model,d)]));
  $('seed').value=player.seed;$('player-name').textContent=player.name;$('gender').value=player.gender;$('equipment').value=player.equipment;
  ...
  for(const type of ['sword','pickaxe']) {
    const tool=player.tools[type];canvas(renderModel(buildToolModel(tool),'front',['tool']),$(`tool-${type}`));
    $(`tool-style-${type}`).value=tool.style;$(`tool-material-${type}`).value=tool.material;
  ...
  $('frame').disabled=action==='idle';$('play-pause').disabled=action==='idle';
  drawFrame();
}
function setAction(next) {
  pauseAnimation();action=next;frameIndex=0;if(next!=='idle')player.equipment=next==='sword_swing'?'sword':'pickaxe';refresh();if(next!=='idle')playAnimation();
}
function replacePlayer(next, honorLocks=true) {
  if(honorLocks) { for(const part of locks) next.parts[part]=structuredClone(player.parts[part]); if(locks.has('hat')||locks.has('body')) next.palette=player.palette; if(locks.has('head')||locks.has('body')) next.skin=player.skin; if(locks.has('body')) next.gender=player.gender; }
  pauseAnimation();action='idle';frameIndex=0;player=next;refresh();
}
...
for(const type of ['sword','pickaxe']) {
  ...
  card.innerHTML=`<canvas id="tool-${type}" width="48" height="48" aria-label="Generated ${type}"></canvas><div class="tool-settings"><span class="eyebrow">${type.toUpperCase()}</span><span id="tool-name-${type}" class="tool-name"></span><select id="tool-style-${type}" aria-label="${title(type)} shape">${TOOL_STYLES[type].map(s=>`<option value="${s}">${title(s)}</option>`).join('')}</select><select id="tool-material-${type}" aria-label="${title(type)} material">${Object.entries(TOOL_MATERIALS).map(([k,m])=>`<option value="${k}">${m.name}</option>`).join('')}</select><div class="tool-buttons"><button id="forge-${type}" aria-label="Forge new ${type}">↻ Forge new</button><button id="try-${type}" aria-label="Try ${type} swing">▶ Try swing</button></div></div>`;
  $('tool-cards').append(card);
  $(`tool-style-${type}`).addEventListener('change',e=>{player.tools[type].style=e.target.value;refresh();});
  $(`tool-material-${type}`).addEventListener('change',e=>{player.tools[type].material=e.target.value;refresh();});
  $(`forge-${type}`).addEventListener('click',()=>{player.tools[type]=generateTool(type,freshSeed());refresh();toast(`Forged a new ${type}. All poses use this same tool.`);});
  $(`try-${type}`).addEventListener('click',()=>setAction(`${type}_swing`));
}
for(const button of document.querySelectorAll('[data-action]'))button.addEventListener('click',()=>setAction(button.dataset.action));
